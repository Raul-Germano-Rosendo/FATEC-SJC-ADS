...
async function salvarContinente() {
  ...
  if (!nome) { toast('Informe o nome do continente', 'error'); return; }
  const item = {
    nome,
    area: val('ct-area') || 0,
...
async function deletarContinente(id) {
  ...
  if (paises.some(p => p.continente === cont?.nome)) {
    toast('Existem países vinculados a este continente!', 'error'); return;
  }
...
function editarContinente(item) {
...
async function salvarEditContinente(id) {
  const item = {
    id,
    nome: val('e-ct-nome'),
    area: val('e-ct-area'),
...
async function renderContinentes() {
  ...
  document.getElementById('st-total-cont').textContent = data.length;
  document.getElementById('st-total-cont-pop').textContent = fmt(data.reduce((a,c) => a+Number(c.populacao||0), 0));
  document.getElementById('st-total-cont-area').textContent = fmt(data.reduce((a,c) => a+Number(c.area||0), 0));
  if (!data.length) {
...
async function seedContinentes() {
  ...
  if (existing.length > 0) { toast('Já existem continentes cadastrados!', 'info'); return; }
  const defaults = [
...
async function preencherSelectContinente() {
  const data = await dbAll('continentes');
  const sel = document.getElementById('p-continente');
  sel.innerHTML = '<option value="">-- Selecione --</option>' + data.map(c => `<option value="${c.nome}">${c.nome}</option>`).join('');
}

async function salvarPais() {
  const nome = val('p-nome');
  if (!nome) { toast('Informe o nome do país', 'error'); return; }
  const item = {
    nome,
...
async function salvarEditPais(id) {
...
async function renderPaises() {
  ...
  const filtrado = busca ? data.filter(p => p.nome.toLowerCase().includes(busca) || (p.capital||'').toLowerCase().includes(busca)) : data;
  const body = document.getElementById('tbody-paises');
  const totalPop = data.reduce((a,p) => a+Number(p.populacao||0), 0);
  const totalArea = data.reduce((a,p) => a+Number(p.area||0), 0);
  document.getElementById('st-total-paises').textContent = data.length;
  document.getElementById('st-total-pop').textContent = fmt(totalPop);
  document.getElementById('st-total-area').textContent = fmt(totalArea);
  if (!filtrado.length) {
...
async function preencherSelectPais() {
  ...
  ['c-pais'].forEach(id => {
    ...
    if (!sel) return;
    sel.innerHTML = '<option value="">-- Selecione --</option>' + data.map(p => `<option value="${p.nome}">${p.nome}</option>`).join('');
  });
}

async function salvarCidade() {
  const nome = val('c-nome');
  if (!nome) { toast('Informe o nome da cidade', 'error'); return; }
  const item = {
    nome,
    pais: val('c-pais'),
...
async function salvarEditCidade(id) {
...
async function renderCidades() {
  ...
  const filtrado = busca ? data.filter(c => c.nome.toLowerCase().includes(busca) || (c.pais||'').toLowerCase().includes(busca)) : data;
  const body = document.getElementById('tbody-cidades');
  document.getElementById('st-total-cidades').textContent = data.length;
  document.getElementById('st-total-capitais').textContent = data.filter(c => c.tipo==='capital').length;
  document.getElementById('st-total-metropoles').textContent = data.filter(c => c.tipo==='metrópole').length;
  if (!filtrado.length) {
  ...
  const tipoBadge = { 'capital':'badge-green', 'metrópole':'badge-blue', 'cidade':'badge-amber', 'município':'badge-gray' };
  body.innerHTML = filtrado.map(c => `
    <tr>
      <td><strong>${c.nome}</strong></td>
      <td>${c.pais||'—'}</td>
...
const WMO = {
  ...
  80:'🌦️ Chuva leve',81:'🌧️ Chuva moderada',82:'⛈️ Chuva violenta',
  95:'⛈️ Trovoada',96:'⛈️ Trovoada com granizo',99:'⛈️ Trovoada violenta'
};
function wmoDesc(code) { return WMO[code] || '🌡️ Condição ' + code; }
...
async function preencherSelectClima() {
  const data = await dbAll('cidades');
  const sel = document.getElementById('cl-cidade');
  if (!sel) return;
  sel.innerHTML = '<option value="">-- Selecionar cidade --</option>' +
    data.filter(c => c.lat && c.lng).map(c => `<option value="${c.lat},${c.lng},${c.nome}">${c.nome} (${c.pais||'?'})</option>`).join('');
}
...
async function consultarClima() {
  ...
  if (!lat || !lng) { toast('Informe latitude e longitude', 'error'); return; }
  await climaCidade(lat, lng, `${lat}, ${lng}`);
}

async function climaCidade(lat, lng, nome='Local') {
  setPage('clima');
  ...
  try {
    ...
    const previsao = d.time.map((dt, i) => {
    ...
    const h = { nome, lat, lng, temp: Math.round(c.temperature_2m), desc: desc.split(' ').slice(1).join(' '), hora: new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}) };
    historicoClima.unshift(h);
    renderHistorico();
...
function renderHistorico() {
  ...
  if (!historicoClima.length) { area.innerHTML = '<p style="color:var(--cinza-600);font-size:14px">Nenhuma consulta realizada.</p>'; return; }
  area.innerHTML = historicoClima.slice(0,10).map(h => `
    <div class="info-row">
...
// ============================================================
// API REST COUNTRIES — buscar país
// ============================================================
async function buscarPaisAPI() {
  const nome = val('p-nome');
  if (!nome) { toast('Informe o nome do país para buscar', 'error'); return; }
  try {
    toast('Buscando dados...', 'info');
    const resp = await fetch(`https://restcountries.com/v3.1/name/${encodeURIComponent(nome)}?fields=name,cca2,capital,population,area,currencies,continents`);
    ...
    const cur = Object.values(p.currencies || {})[0];
...
// INIT
// ============================================================
async function seedDadosInternet() {
  toast('Iniciando preenchimento via internet...', 'info');

  // Limpa dados existentes (mantém continente/pais/cidade coerentes)
  const existingContinentes = await dbAll('continentes');
  const existingPaises = await dbAll('paises');
  const existingCidades = await dbAll('cidades');

  for (const c of existingCidades) await dbDel('cidades', c.id);
  for (const p of existingPaises) await dbDel('paises', p.id);
  for (const ct of existingContinentes) await dbDel('continentes', ct.id);

  // Continentes padrão (para manter o select funcional)
  const defaults = [
    { nome:'Africa', area:30370000, populacao:1460481772, paises:54, notas:'População estimada (seed)' },
    { nome:'Asia', area:44579000, populacao:4753079727, paises:49, notas:'População estimada (seed)' },
    { nome:'Europe', area:10530000, populacao:748000000, paises:44, notas:'População estimada (seed)' },
    { nome:'North America', area:24709000, populacao:597634000, paises:23, notas:'População estimada (seed)' },
    { nome:'South America', area:17840000, populacao:434260000, paises:12, notas:'População estimada (seed)' },
    { nome:'Oceania', area:8600000, populacao:44491724, paises:14, notas:'População estimada (seed)' },
    { nome:'Antarctica', area:14200000, populacao:0, paises:0, notas:'Sem população permanente (seed)' },
  ];

  for (const d of defaults) await dbAdd('continentes', d);

  // Seeds de países por continente (limite por performance). Ajuste se quiser.
  const selection = {
    ...
    'Asia': ['China', 'India', 'Japan', 'Saudi Arabia', 'Turkey'],
    'Oceania': ['Australia', 'New Zealand', 'Papua New Guinea', 'Fiji', 'Samoa'],
    'Antarctica': [],
  };

  // Helpers
  const isoToContinentKey = (continentsArr) => {
    // restcountries retorna cont. exatas como strings. Usamos fallback.
    if (!continentsArr?.length) return null;
    const c0 = continentsArr[0];
    // alguns endpoints retornam em inglês: normalize e tenta mapear
    return c0;
  };

  let insertedPaises = 0;

  for (const [contKey, paisList] of Object.entries(selection)) {
    for (const nomePais of paisList) {
      // Pequeno throttle
      await new Promise(r => setTimeout(r, 250));

      try {
        const url = `https://restcountries.com/v3.1/name/${encodeURIComponent(nomePais)}?fields=name,cca2,capital,population,area,currencies,continents`;
        const resp = await fetch(url);
        if (!resp.ok) continue;
        const data = await resp.json();
        const p = data?.[0];
        ...
        // Continente do país
        const cont = isoToContinentKey(p.continents);
        // Se não bater com nosso set, tenta aproximar pelo primeiro
        const continenteNome = cont || contKey;

        const moedaObj = Object.values(p.currencies || {})[0];
        const moeda = moedaObj ? `${moedaObj.name} (${Object.keys(p.currencies)[0]})` : '';

        const itemPais = {
          nome: p.name.common,
          codigo: p.cca2,
          continente: continenteNome,
          capital: (p.capital && p.capital[0]) ? p.capital[0] : '',
          populacao: p.population || 0,
          area: p.area || 0,
          moeda
        };

        // verifica se continente existe; se não, usa fallback contKey
        const contExists = (await dbAll('continentes')).some(ct => ct.nome === itemPais.continente);
        if (!contExists) itemPais.continente = contKey;

        await dbAdd('paises', itemPais);
        insertedPaises++;
      } catch (e) {
  ...
  toast(`Seed Internet concluído. Países: ${insertedPaises}`);
